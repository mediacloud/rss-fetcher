...
2. Make sure `DATABASE_URL` is set (if have a Dokku instance; you can run `export DATABASE_URL=$(./dokku-scripts/dburl.sh pbudne-rss-fetcher)`)
...
6. `git add` the new migration file and commit it!
...
possible), reverse the old migration, remove the file autogenerate a
new migration file!
...
index definitions after each {up,down}grade; type `\q` or CTRL/D will quit psql
