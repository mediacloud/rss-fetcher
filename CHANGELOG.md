...
Major raking by Phil Budne
    ...
    * All script startup logging includes script name and Dokku
        deployed git hash, followed by ONLY logging the configuration that
        is referenced.

    * All scripts log to BASE/storage/logs/APP.DYNO.log;
        files turned over at midnight (to filename.log.YYYY-MM-DD),
